...
export const generateAudio = async (content) => {
    try {
        ...
        const tempDir = path.join('public', '..', 'temp'); // Đường dẫn tuyệt đối
        ...
        const audioUpload = await cloudinary.uploader.upload(tempAudioPath, {
            resource_type: "video", // Dùng video cho file âm thanh
        });
...
export const generateSubtitles = async (content) => { 
    try {                
        ...
        const subtitleUpload = await cloudinary.uploader.upload(tempVttPath, {
