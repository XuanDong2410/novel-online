...
<template>
  <ClientOnly>
    ...
    >
      <!-- :toggle="undefined" -->
      <template #title>
        <span id="header-title" class="sr-only">Novel Online</span>
      ...
      <UCard class="overflow-y-auto flex flex-col p-1 cursor-pointer">
        ...
        <template v-if="auth.isAuthenticated">
          ...
          <ul class="space-y-4 text-gray-700 dark:text-gray-200 mb-8">
            <li v-if="auth.user?.role === 'admin'" class="flex items-center gap-2 hover:text-primary-600 transition-colors">
              <NuxtLink to="/admin">
