          ...
          <div class="max-w-4xl mx-auto px-6 py-8">
            ...
            <div v-if="isAudioMode && currentAudio" class="mb-8">
              <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                ...
                >
                  <track
                    v-if="currentAudio.subtitle?.url"
                    kind="subtitles"
                    :label="currentAudio.subtitle.language || 'Tiếng Việt'"
                    :src="currentAudio.subtitle.url"
                    default
                  >
                </audio>
                <div
                  v-if="isAudioMode && currentSubtitleText"
                  class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-gray-700 dark:text-gray-300 text-center"
                >
                  {{ currentSubtitleText }}
                </div>
                <div
                  v-else-if="isAudioMode && !currentSubtitleText && currentAudio.subtitle?.url && !loadingSubtitles"
                  class="mt-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg text-gray-800 dark:text-gray-200 text-center text-sm"
                >
                  Không có phụ đề tại thời điểm hiện tại.
                </div>
                <div
                  v-else-if="isAudioMode && loadingSubtitles"
                  class="mt-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg text-gray-800 dark:text-gray-200 text-center text-sm"
                >
                  Đang tải phụ đề...
                </div>
                <div
                  v-else-if="isAudioMode && !currentAudio.subtitle?.url"
                  class="mt-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg text-gray-800 dark:text-gray-200 text-center text-sm"
                >
                ...
                >
                  <span
                    :class="[
                      ...
                      index === selectedSentenceIndex && index !== currentSentenceIndex ? 'bg-blue-100 border-l-4 border-blue-500' : '',
                      index < currentSentenceIndex && (isAudioMode ? audioSyncEnabled : isTTSPlaying) ? 'read-sentence' : ''
                    ]"
            ...
            <div class="flex items-center justify-between mt-12 pt-8 border-t border-gray-200 dark:border-gray-800">
              <UButton
                ...
                variant="outline"
                :to="`/novel/${novelId}/${previousChapter._id}`"
              >
                Chương trước
              </UButton>
              <div v-else />
              <UButton
                ...
                trailing
                :to="`/novel/${novelId}/${nextChapter._id}`"
              >
                Chương tiếp
              </UButton>
              <div v-else />
        ...
        <template #body>
          <UCard>
            <div class="space-y-6">
              ...
              <div>
                <label class="block text-sm font-medium mb-2">Giọng đọc (nếu có)</label>
...
// Khai báo các composable để lấy dữ liệu
const { getChapterById, getChaptersByNovelId } = useChapters()
// const { getNovelById } = useNovels()

// 1. Lấy dữ liệu chương hiện tại (selectedChapter)
// Sử dụng useAsyncData vì getChapterById là một hàm async tùy chỉnh.
const { data: selectedChapter, pending, error } = await useAsyncData<Chapter>(
  `chapter-${chapterId}`, // Key duy nhất cho dữ liệu này, hữu ích cho caching
  async () => {
    const result = await getChapterById(chapterId)
    const raw = result.data.value // Lấy giá trị thực tế từ ref
    return {
      ...raw,
      novelId: typeof raw?.novelId === 'string' ? raw.novelId : (raw?.novelId?._id ?? ''),
      audios: Array.isArray(raw?.audios) ? raw.audios.filter(a => typeof a === 'object' && a !== null) as IAudio[] : []
    }
  },
  {
    // Tùy chọn: re-fetch nếu chapterId thay đổi
    watch: [() => route.params.chapter],
    immediate: true // Chạy fetch ngay lập tức khi component được mount
  }
)

// 2. Lấy dữ liệu truyện (novelData)
// Sử dụng useFetch nếu bạn gọi trực tiếp API HTTP
const { data: novelData } = await useFetch<Novel>(
  `http://localhost:5000/api/v1/novel/${novelId}`,
  {
    method: 'GET',
    credentials: 'include',
    key: `novel-${novelId}`, // Key duy nhất
    // Tùy chọn: re-fetch nếu novelId thay đổi
    watch: [() => route.params.id],
    immediate: true
  }
)

// 3. Lấy danh sách các chương của truyện (chapters)
// Sử dụng useAsyncData vì getChaptersByNovelId là một hàm async tùy chỉnh.
const { data: chapters, pending: chaptersPending, error: chaptersError } = await useAsyncData<Chapter[]>(
  `chapters-for-novel-${novelId}`, // Key duy nhất
  async () => {
    const result = await getChaptersByNovelId(novelId)
    // Đảm bảo result.data.value là một mảng trước khi map
    return (result.data.value || []).map((chapter: any) => ({
      ...chapter,
      audios: Array.isArray(chapter.audios) ? chapter.audios.filter((a: IAudio) => typeof a === 'object' && a !== null) as IAudio[] : []
    })) as Chapter[]
  },
  {
    // Tùy chọn: re-fetch nếu novelId thay đổi
...
// Audio/Subtitle specific reactive data
interface SubtitleCue {
  start: number
  end: number
  text: string
}
const allSubtitles = ref<Record<string, SubtitleCue[]>>({})
...
const currentAudio = computed(() => {
  ...
  return selectedChapter.value.audios[0]
})
...
async function loadVttFile(audioId: string, url: string) {
  ...
  try {
    if (import.meta.client) {
      console.log(`Loading VTT for audio ${audioId}:`, url)
    }
    const response = await fetch(url)
    const vttText = await response.text()
    const cues = parseVtt(vttText)
    allSubtitles.value[audioId] = cues
    sentences.value = cues.map(cue => ({
      text: cue.text,
      startTime: cue.start,
      endTime: cue.end
    }))
    if (import.meta.client) {
      console.log('Subtitles loaded:', cues)
    }
  } catch (error) {
    console.error(`Error loading or parsing VTT for audio ${audioId}:`, error)
    allSubtitles.value[audioId] = []
  } finally {
...
const selectSentence = (index: number) => {
  ...
  if (isAudioMode.value && audioSyncEnabled.value && audioPlayer.value && sentences.value[index]) {
    audioPlayer.value.currentTime = sentences.value[index].startTime
    if (!isPlaying.value) {
      audioPlayer.value.play()
      isPlaying.value = true
    }
  } else if (isTTSMode.value) {
    stopTTS()
...
const onTimeUpdate = () => {
  if (audioPlayer.value) {
    ...
    if (isAudioMode.value && audioSyncEnabled.value && currentAudio.value && allSubtitles.value[currentAudio.value._id]) {
      const currentCues = allSubtitles.value[currentAudio.value._id]
      ...
      const currentSentenceFromCues = sentences.value.findIndex(
        s => currentTime.value >= s.startTime && currentTime.value < s.endTime
      )
      if (currentSentenceFromCues !== -1) {
        currentSentenceIndex.value = currentSentenceFromCues
        if (ttsAutoScroll.value) {
          scrollToCurrentSentence()
        }
      }
    } else if (isAudioMode.value && audioSyncEnabled.value) {
...
const onAudioEnded = () => {
  ...
  if (autoNextChapter.value && nextChapter.value) {
    navigateTo(`/novel/${novelId.value}/${nextChapter.value._id}`)
  }
}
...
const toggleMode = () => {
  ...
  if (isAudioMode.value && currentAudio.value) {
    if (audioPlayer.value) {
      audioPlayer.value.src = currentAudio.value.audioFileUrl
      audioPlayer.value.load()
    }
    if (currentAudio.value.subtitle?.url && !allSubtitles.value[currentAudio.value._id]) {
      loadVttFile(currentAudio.value._id, currentAudio.value.subtitle.url)
    }
  }
}
...
const skipBackward = () => {
  if (audioPlayer.value) {
    audioPlayer.value.currentTime = Math.max(0, audioPlayer.value.currentTime - 15)
  }
}

const skipForward = () => {
  if (audioPlayer.value) {
    audioPlayer.value.currentTime = Math.min(duration.value, audioPlayer.value.currentTime + 15)
  }
}
...
const loadTTSVoices = () => {
  ...
  if (ttsVoices.value.length > 0 && !selectedTTSVoice.value) {
    ...
    if (import.meta.client) {
      console.log('TTS voices loaded:', ttsVoices.value)
    }
  }
}
...
const toggleTTSMode = () => {
  ...
  if (isTTSMode.value) {
    ...
    if (import.meta.client) {
      console.log('TTS mode enabled, sentences:', sentences.value)
    }
  } else {
    stopTTS()
  }
...
const saveTTSSettings = () => {
  localStorage.setItem('tts-settings', JSON.stringify({
    selectedVoice: selectedTTSVoice.value ? { name: selectedTTSVoice.value.name, lang: selectedTTSVoice.value.lang } : null,
    rate: ttsRate.value,
...
onMounted(() => {
  if (import.meta.client) {
    console.log('Component mounted, selectedChapter:', selectedChapter.value)
  }
  if (selectedChapter.value?.content) {
    if (!currentAudio.value?.subtitle?.url) {
      parseSentences(selectedChapter.value.content)
      if (import.meta.client) {
        console.log('Sentences parsed:', sentences.value)
      }
    }
  }
  initializeTTS()
  ...
  if (isAudioMode.value && currentAudio.value) {
    audioPlayer.value = new Audio(currentAudio.value.audioFileUrl)
    audioPlayer.value.volume = volume.value
    audioPlayer.value.playbackRate = parseFloat(playbackSpeed.value)
...
watch(playbackSpeed, (newSpeed) => {
  if (audioPlayer.value) {
    audioPlayer.value.playbackRate = parseFloat(newSpeed)
  }
})
...
watch(selectedChapter, (newChapter) => {
  if (newChapter) {
    if (import.meta.client) {
      console.log('Chapter changed:', newChapter)
    }
    if (audioPlayer.value) {
      audioPlayer.value.pause()
      isPlaying.value = false
    ...
    if (isAudioMode.value && currentAudio.value) {
      ...
      if (currentAudio.value.subtitle?.url) {
        ...
        parseSentences(newChapter.content ?? '')
      ...
      sentences.value = splitIntoSentences(newChapter.content ?? '')
      currentSentenceIndex.value = 0
    } else {
      parseSentences(newChapter.content ?? '')
    }
...
watch(ttsVoices, (newVoices) => {
  const savedTTSSettings = localStorage.getItem('tts-settings')
  if (savedTTSSettings) {
    const settings = JSON.parse(savedTTSSettings)
    if (settings.selectedVoice) {
      selectedTTSVoice.value = newVoices.find(
        voice => voice.name === settings.selectedVoice.name && voice.lang === settings.selectedVoice.lang
      ) || null
    }
  }
}, { deep: true })
...
onUnmounted(() => {
  if (audioPlayer.value) {
    ...
    audioPlayer.value.removeEventListener('loadedmetadata', onAudioLoaded)
