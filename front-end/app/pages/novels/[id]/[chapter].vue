<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div class="flex items-center gap-2">
          ...
          <div v-if="isAudioMode || isTTSMode" class="flex items-center gap-2 px-3 py-1 bg-gray-100 dark:bg-gray-800 rounded-lg">
            <UButton
              ...
              variant="ghost"
              size="xs"
              @click="isTTSMode ? toggleTTS() : toggleAudio()"
            />
          ...
          <UButton
            ...
            variant="ghost"
            size="sm"
            @click="console.log(showSettings), showSettings = !showSettings"
          />
        </div>
      </div>
    ...
    <div class="flex">
      ...
      >
        <div class="max-w-4xl mx-auto px-6 py-8">
          ...
          <div v-if="isAudioMode && currentChapter.hasAudio" class="mb-8">
    ...
    <UModal v-model="showAudioSettings" title="Audio Settings">
      <template #header>
        <h3 class="text-lg font-semibold">
          Cài đặt âm thanh
        </h3>
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          <UButton variant="outline" @click="showAudioSettings = false">
            Hủy
          </UButton>
          <UButton @click="saveAudioSettings">
            Lưu
          </UButton>
        </div>
      </template>
    ...
    <!-- TTS Settings Modal -->
    <UModal v-model="showTTSSettings" title="TTS Settings">
      <template #body>
        <TTS
          :voices="ttsVoices"
          :selected-voice="selectedTTSVoice"
...
console.log(showSettings, showAudioSettings, showTTSSettings)
// TTS related
const isTTSMode = ref(false)
const ttsVoices = ref([])
const selectedTTSVoice = ref(null)
...
const chapters = ref([
  { id: 1, title: 'Khởi đầu của một hành trình', duration: '15:30', hasAudio: true, isRead: false },
  { id: 2, title: 'Cuộc gặp gỡ định mệnh', duration: '18:45', hasAudio: true, isRead: false },
  { id: 3, title: 'Sức mạnh thức tỉnh', duration: '22:10', hasAudio: true, isRead: false },
  { id: 4, title: 'Thử thách đầu tiên', duration: '16:20', hasAudio: false, isRead: false },
  { id: 5, title: 'Bí mật được tiết lộ', duration: '20:35', hasAudio: true, isRead: false }
])
...
const saveTTSSettings = (settings) => {
  selectedTTSVoice.value = settings.selectedVoice
  ttsRate.value = settings.rate
  ttsPitch.value = settings.pitch
  ttsVolume.value = settings.volume
  ttsAutoScroll.value = settings.autoScroll !== false
  autoNextChapter.value = settings.autoNext !== false
...
// Watch for chapter changes to update sentences
