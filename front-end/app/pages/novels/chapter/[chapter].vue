...
// Log route params on client side
if (process.client) {
  console.log('Route params:', { novelId: novelId.value, chapterId: chapterId.value })
}
...
const { data: selectedChapter, pending, error } = await useAsyncData<Chapter>(
  ...
  async () => {
    try {
      const result = await getChapterById(chapterId.value)
      const raw = result.data.value
      if (!raw) throw new Error('No chapter data returned')
      return {
        ...raw,
        _id: raw?._id ?? '',
        novelId: typeof raw?.novelId === 'string' ? raw.novelId : (raw?.novelId?._id ?? ''),
        audios: Array.isArray(raw?.audios)
          ? raw.audios.filter((a): a is IAudio => typeof a === 'object' && a !== null)
          : []
      }
    } catch (err) {
  ...
  { server: false } // Ensure client-side fetching for consistency
)

// Fetch novel data reactively based on selectedChapter
const novelData = ref<Novel | null>(null)
// watch(
//   () => novelId,
//   async (newNovelId) => {
//     if (newNovelId && typeof newNovelId === 'string') {
//       if (process.client) {
//         console.log('Fetching novel with ID:', newNovelId)
//       }
//       try {
//         const response = await $fetch<Novel>(`http://localhost:5000/api/v1/novel/${novelId.value}`, {
//           method: 'GET',
//           credentials: 'include'
//         })
//         novelData.value = response
//         if (process.client) {
//           console.log('Novel data fetched:', response)
//         }
//       } catch (err) {
...
// Fetch chapters reactively based on novelId
...
watch(
  ...
  async (newNovelId) => {
    if (newNovelId) {
      if (process.client) {
        console.log('Fetching chapters for novel ID:', newNovelId)
      }
      try {
        const result = await getChaptersByNovelId(newNovelId)
        chapters.value = (result.data.value || []).map((chapter: any) => ({
          ...chapter,
          audios: Array.isArray(chapter.audios)
            ? chapter.audios.filter((a: IAudio) => typeof a === 'object' && a !== null)
            : []
        }))
        if (process.client) {
